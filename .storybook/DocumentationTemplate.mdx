...
You can try configuring your `tsconfig.json` to look for the exported types from this package.
You can also manually create a (or add to an existing) `.d.ts` file and copy the contents of the
exported types in `/dist/types.d.ts`.
